        ...
        with IdealSchemaEditor(connection=connections['ideal']) as schema_editor:
            ...
            if big:
                with tempfile.NamedTemporaryFile(mode='w+', suffix='.csv', delete=True) as tmp_file:
                    ...
                    # Пример передачи файла в BULK INSERT (через pyodbc)
                    tmp_file.seek(0)  # Перемещаем указатель в начало
                    bulk_insert_to_sqlserver(tmp_file.name, model)  # Ваша функция загрузки
            else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            match form.cleaned_data['form_name']:
                ...
                case 'implant':
                    #process_implant_file(form.cleaned_data['file'], request)
        ...
        print(0)
    return render(request, "domestic_service/dm_page.html", context={})