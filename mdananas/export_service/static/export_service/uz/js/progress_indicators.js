...
function trackProgress(active_form, file_name) {
    ...
    const progressInterval = setInterval(() => {
        fetch('get-progress/')
            ...
            .then(data => {
                if (file_name === 'sales') {
                    percentElement.textContent = data.uz_sales_file_progress + '%';
                    if (!data.uz_sales_file_active) {
                        clearInterval(progressInterval);
                        percentElement.textContent = '100%';
                    }
                }
                if (file_name === 'kidpoint') {
                    percentElement.textContent = data.uz_kidpoint_file_progress + '%';
                    if (!data.uz_kidpoint_file_active) {
                        clearInterval(progressInterval);
                        percentElement.textContent = '100%';
                    }
                }
...
window.addEventListener('load', function() {
    ...
    addFileInputChangeListener(document.getElementById('kidpoint-file-upload-form'), 'kidpoint');
});